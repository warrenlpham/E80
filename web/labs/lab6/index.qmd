...
Submission Sheet: [html](/labs/lab6/submission.qmd) [Word](/labs/lab6/submission.docx)
...
You will be using COMSOL Multiphysics software in this lab. COMSOL is complicated software, and you will struggle to finish the lab if you do not read about its use and practice with it during prelab. You may practice using COMSOL and go through Granny Spark's COMSOL tutorials as part of prelab. However, you CANNOT run COMSOL simulations on LAB-RELATED objects outside of your designated lab time.  These objects include the cylindrical objects tested in the wind tunnel, the "flat plate wing" and the NACA 6412 wing. You also cannot use COMSOL during any other section's lab time slot because that can deprive students in lab of COMSOL licenses.
...
This is a rotation lab, so not all teams will be working on this lab in the same week.  Even numbered teams (12, 24, 36, etc.) will do this lab in week 6, while Odd numbered teams (12, 24, 36, etc.) work on [lab 5](lab5/index.qmd).  Odd numbered teams will do this lab in week 7, while Even numbered teams work on lab 5.  Be sure to watch the correct videos, do the correct quiz, and put in prelab effort for the correct lab!
...
3. Adjust the fan speed with the knob on the RPM Controller while reading the RPM from the Fan Speed Display.  Adjust the fan speed from 20% to 90% of max fan speed (in increments of 10%). At each fan speed:   
...
3. For each of "Nose #1", "Nose #2" and "Nose #3"
   ...
   4. Measure the drag force on the object from 20% to 90% of max fan speed (in increments of 10%).  
...
4. Record necessary calibration information, then record drag force on the object from 20%-90% of max fan speed (in increments of 10%).
