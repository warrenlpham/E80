...
Be sure to read the [Lab Overview](../lab_overview/index.qmd) to learn about safety requirements, expensive equipment handling and prelab rules.  You must review all SAFETY QUESTION boxes below as part of prelab.  If you ever reach a safety question box and either can't answer it or feel unsure, grab an instructor.


## 1. Battery

Your AUV will need power, and we provide that with a battery pack. The battery pack used in E80 is a 3000 mAh to 3500 mAh, 30 C, 11.1 V LiPo battery pack. To charge the batteries, we will use a [HiTec x1 Mini charger](https://hitecrcd.com//images/products/pdf/419_X1mini_Manual_BAS-V2-Web.pdf) or a [UP60AC multi charger](http://www.hrpdealer.com/pdf/manuals/UPTUP60AC%20Manual.pdf). Read the manual on how to use the chargers. Here are some helpful [images](https://drive.google.com/open?id=1K1DSOdeqAjU_JU-sTOdiBUnwMmpwUTPP) showing correct charger use.

:::{.callout-warning collapse="true"}
## SAFETY QUESTION: What is the maximum voltage output of these battery packs?

12.6V.  Don't charge above this level.  This is calculated by observing that there are 3 cells in your battery (i.e.: it is 3S), and that each cell maxes out at 4.2V.
:::

:::{.callout-warning collapse="true"}
## SAFETY QUESTION: What is the nominal voltage output of these battery packs?

11.1V.  Don't discharge below this level.  Li-ion batteries discharge quickly when their voltage sinks below the nominal level, so keep a close watch on this.  This is calculated by observing that there are 3 cells in your battery (i.e.: it is 3S), and that each cell has a nominal voltage of 3.7V.
:::

:::{.callout-warning collapse="true"}
## SAFETY QUESTION: Should the balancer cable be plugged in during charging?

Yes, with the black cable aligned with the minus sign.  **Failing to do this can cause the battery to light on fire.**
:::

:::{.callout-warning collapse="true"}
## SAFETY QUESTION: Should I use a LiPo safe bag while charging?

Yes, always.  This keeps you, other students, and the lab safe in case of battery fire.
:::

Request a battery and a LiPo safe bag from an instructor. You will need to check out and set up your battery for charging as soon as you start the lab. ASK AN INSTRUCTOR OR PROCTOR TO CHECK YOUR RECHARGE SETUP. Charge the battery up to its nominal voltage, NOT its maximum voltage, which means that you will need to periodically monitor the voltage on the battery and stop the charging at the appropriate time. 
...
You will use the Arduino language to program your microcontroller, the [Teensy 4.0](https://www.pjrc.com/store/teensy40.html). 

Install the Arduino integrated development environment (IDE) software and download the E80 reference code.

1. If your computer doesn't have Arduino software installed, go to [the Arduino download page](https://www.arduino.cc/en/software)  and download the latest version of the Arduino IDE (2.2.1 as of this writing). 
2. Since version 2.0.0, the Teensy is supported directly in the Arduino software. Follow the instructions [here](https://www.pjrc.com/arduino-ide-2-0-0-teensy-support/) to add the required board support packages to the Arduino IDE. 
3. If you have not done so yet, complete the Github Desktop Setup in [Lab 0](../lab0/index.qmd) and clone the [E80 codebase](https://github.com/HMC-E80/E80-SP2023).
...
6. Confirm that you can compile and upload the starter code to your Teensy.  Since your Teensy is not plugged into your motherboard here, this code will do little.  It is fully tested in Section 4.
