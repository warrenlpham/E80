...
Submission Sheet: [html](/labs/lab2/submission.qmd) [Word](/labs/lab2/submission.docx)

Prelab Help Sheet: [html](/labs/lab2/prelab.qmd)
...
1. Connect a  200 Hz, 1 Vpp, +0.4 V offset signal from a signal generator to the analog pin A0 and run the following sketch on your Teensy: `matlab_logging.ino`. 
...
Finally, answer this question about your second set of data

1. What signal does your Teensy observe? Why? (Hint: you may need to think back to some parts of E79 to figure this one out.)

## Resistor Dividers and Instrument Loading {#sec-dividers}

This section is designed to give you practice on two skills: accounting for instrument loading (i.e.: source and load impedance), and calculating uncertainties using error propagation. @fig-divider and @tbl-experiment are essential for this section:
...
You will be asked to build the circuits specified in the table and to measure the output voltage with the devices in the "Measure With" column, but some of the devices in the "Measure With" column may be unfamiliar to you.  The devices are described below, and datasheet links are provided for each of them.  To finish this section, you will need to read these datasheets to find the input impedance and the uncertainty of each device.
- "Multimeter" refers to the multimeter chained to your desk. Our multimeters are the [Greenlee DM-210A Digital Multimeter](https://cdn.greenlee.com/resources/media?key=7fe82ac7-208f-4c14-910c-a522a2949b3c&languageCode=en&type=document). Since the $V_{out}$ node will very with time, you should be sure to measure it with your multimeter to AC mode.  The uncertainty listings for multimeters can use unusual units (e.g.: dgt and rdg), so don't forget about [this document on multimeter specifications](https://www.designworldonline.com/how-to-determine-digital-multimeter-accuracy/) that we also link on the lectures page.
- "1x scope probe" refers to using a BNC cable with hooks directly connected to an oscilloscope input.  BNC cables don't have datasheets worth reading for this lab because they're just wires, so they have a low series impedance.  However, the oscilloscope itself (an [Agilent 2024A](http://literature.cdn.keysight.com/litweb/pdf/75015-97045.pdf), which has this [datasheet](http://literature.cdn.keysight.com/litweb/pdf/5990-6618EN.pdf)) has an input impedance and accuracy that you will need to look up. You can also create a 1x probe by using an oscilloscope probe with a switch set to 1x mode, and it will behave the same way as a BNC cable.  When you are using a 1x probe, you must change the channel setting of the oscilloscope to be 1x.
- "10x scope probe" refers to connecting a standard oscilloscope probe (set to 10x mode if there's a switch) to an oscilloscope input.  The gray scope probes in the lab are [Keysight N2842A](https://assets.testequity.com/te1/Documents/pdf/keysight/N28xxA-ds.pdf) probes, and the black scope probes in 10x mode have a similar input impedance.  Note that the input impedance of a scope probe is added _in series_ with that of the oscilloscope itself, so the datahsheet may report a total "terminated impedance" that includes the input impedance of the oscilloscope.
...
6. Explain any differences between your measured voltages and your two sets of predicted voltages. Do the measured voltages lie within the predicted confidence intervals? Are there any lessons here on how to increase the accuracy of predicted measurements?
...
| Z1           | Z2           | Test Frequencies |
|--------------|--------------|------------------|
| 10 k$\Omega$ | 0.47 $\mu$F  | 1 Hz, 10 Hz, 100 Hz, 1 kHz, 10 kHz, 100 kHz | 
| 0.47 $\mu$F  | 10 k$\Omega$ | 1 Hz, 10 Hz, 100 Hz, 1 kHz, 10 kHz, 100 kHz |
| 220 $\Omega$ | 1 $\mu$F     | 1 Hz, 10 Hz, 100 Hz, 1 kHz, 10 kHz, 100 kHz |
| 10 M$\Omega$ | 1000 pF      | 1 Hz, 10 Hz, 100 Hz, 1 kHz, 10 kHz, 100 kHz |

: Table of RC divider measurements {$tab-rc}

Build the circuits in table @tab-rc in the same manner as the dividers in @sec-dividers, then repeat the predictions and measurements as in @sec-dividers. Vary the input frequency to each of the values listed in the "Test Frequencies" column as you perform these measurements. Use an input amplitude of 5Vpp and, if using the Teensy, be sure to offset the signal. Note that the multimeter can measure capacitance. Measure the output voltages with any appropriate equipment (justify your choice). Measure the phase difference between the input and output Voltages where possible. Explain the results of your measurements and possible function for each circuit. Consider carefully in what (graphical) format(s) you want to present your results. 
